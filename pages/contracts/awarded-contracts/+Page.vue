...
type FilterForm = {
    ...
    contractor: string;
    numChanges: string;
    smallContract: "true" | "false";
    typeProcedureId: string;
    typeOfferId: string;
    typeFrameworkAgreementId: string;
    moreThanEstimatedValue: string;
    lessThanEstimatedValue: string;
...
const selectedContract = ref<AwardedContractItem | null>(null);
...
const queryBase = computed<AwardedContractsQuery>(() => {
    ...
    return {
        ...
        numChanges: toNonNegativeNumber(debouncedFilters.value.numChanges),
        smallContract: debouncedFilters.value.smallContract === "true",
...
const {
    ...
    queryKey: computed(() => ["awarded-contracts", queryBase.value]),
    queryFn: async ({ pageParam }) =>
        contractsService.awarded.getAwardedContracts({
            ...queryBase.value,
...
const activeFiltersCount = computed(
    () =>
        [
            ...
            filters.contractor,
            filters.numChanges,
            filters.smallContract === "true" ? "1" : "",
            filters.typeProcedureId,
            filters.typeOfferId,
            filters.typeFrameworkAgreementId,
            filters.moreThanEstimatedValue,
            filters.lessThanEstimatedValue,
...
const openDrawer = (contract: AwardedContractItem) => {
