...
const columns = computed<ColumnDef<RealisedContractItem>[]>(() => [
    {
    ...
    },
    {
        ...
        header: () => t("contracts.table.realisedValue"),
        cell: (info) =>
            formatCurrency(
                Number(info.getValue() ?? 0),
                selectedCurrency.value,
    ...
    },
    {
        ...
        header: () => t("contracts.table.paidValue"),
        cell: (info) =>
            formatCurrency(
                Number(info.getValue() ?? 0),
                selectedCurrency.value,
...
const getSortState = (header: Header<RealisedContractItem, unknown>) =>
...
<template>
  <div class="grid min-h-0 grid-rows-[auto_minmax(0,1fr)] overflow-hidden border border-muted bg-surface">
    <div
      ...
      @scroll="onScroll"
    >
      <table class="w-full min-w-352 table-fixed border-separate border-spacing-0 text-sm">
        <colgroup>
          <col class="w-36" />
          <col class="w-[20rem]" />
          <col class="w-[20rem]" />
        ...
        <tbody>
          ...
          <tr v-else-if="isLoading && rows.length === 0">
            <td colspan="7" class="px-4 py-8 text-center text-accent">
              {{ t('common.loading') }}
            </td>
          </tr>
